...
withr::with_options(
  ...
  code = expect_identical(
    ...
    info = "`skip_if_not_installed()` skips if `pkg` can be loaded, but is not at least `minimum_version`"
  )
)
...
withr::with_options(
  ...
  code = expect_null(
    ...
    info = "`skip_if_not_installed()` does not skip if `pkg` is installed and at least `minimum_version`"
  )
)
...
# `skip_on_os()`
systems <- c('windows', 'mac', 'linux', 'sunos', 'solaris')
...
nonarch <- paste(sample(letters, size = 10L, replace = TRUE), collapse = '')
...
withr::with_options(
  new = op,
  code = expect_identical(
    current = skip_on_os(os = union(sysos, systems)),
    target = sprintf(fmt = "On %s", msgos),
...
withr::with_options(
  ...
  code = expect_null(
    ...
    info = sprintf(
      fmt = "`skip_on_os()` does not skip when under `os` ('%s'), but on different `arch` (system '%s' vs non-arch '%s')",
      sysos,
      sysarch,
      nonarch
    )
...
for (os in setdiff(systems, sysos)) {
  withr::with_options(
    ...
    code = expect_null(
      ...
      info = sprintf(
        fmt = "`skip_on_os()` does not skip when not under `os` ('%s'), even when on `arch` ('%s')",
        os,
        sysarch
      )
    )
